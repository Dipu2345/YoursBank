 ...
 * @author DEBASHIS
 */
public class ChangeUserIdToNew extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String email =(String) sson1.getAttribute("changeUserEmail");
            String user_id = request.getParameter("confirm_id");
            //code to change username
            ActivityDao ad = new ActivityDao(DbConnect.getConnect());
            if(ad.UpdateUserName(email, user_id))
                out.println("success");
            else
                out.println("error");     
        }
